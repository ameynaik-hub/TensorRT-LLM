...
class Eagle3OneModelWorker(nn.Module):
    ...
    def sample_and_accept_draft_tokens(
        ...
        # if logits.device == "cuda:0":
        target_tokens = torch.ops.ARGMAX.argmax_cga(logits)
        # if logits.shape[0] == 1:
        #     target_tokens = torch.ops.ARGMAX.argmax_cga(logits[-1])
        # else:
        #     # print(f"logits: {logits.shape}, {logits.dtype}, {logits.device}")
        #     target_tokens = torch.argmax(logits, dim=-1)
        #     # print(f"target_tokens: {target_tokens.shape}, {target_tokens.dtype}, {target_tokens.device}")
    ...
    def draft_decoder(
        ...
        '''
        # if logits.shape[0] == 1:
        #     draft_tokens = torch.ops.ARGMAX.argmax_cga(logits[-1])
        # else:
        #     draft_tokens = torch.argmax(logits, dim=-1) # [num_tokens]
        draft_tokens = torch.ops.ARGMAX.argmax_cga(logits)
        # draft_tokens = torch.ops.ARGMAX.argmax_cga(logits)
        # print(f"draft_decoder: logits.shape: {logits.shape}, logits.dtype: {logits.dtype}, logits.device: {logits.device},draft_tokens.shape: {draft_tokens.shape}, draft_tokens.dtype: {draft_tokens.dtype}, draft_tokens.device: {draft_tokens.device}")
